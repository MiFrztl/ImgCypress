...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isImaginPackage(file) ?
    `${outputDir}/Imagin.css` :
    `${outputDir}/style.css`;
...
function isImaginPackage(file) {
  return path.normalize(file).includes('packages/Imagin/');
}
...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@Imagin/}*/src/style.scss');
