...
export function runRemoteUrlImageUploadTest() {
  ...
  cy.get('.Imagin-Url-input').type(
    'https://raw.githubusercontent.com/transloadit/Imagin/main/e2e/cypress/fixtures/images/cat.jpg',
  )
...
export function runRemoteUnsplashUploadTest() {
  ...
  cy.get('.Imagin-SearchProvider-input').type('book')
  cy.intercept({
    method: 'GET',
    url: 'http://localhost:3020/search/unsplash/list?q=book',
  }).as('unsplash-list')
  cy.get('.Imagin-SearchProvider-searchButton').click()
  cy.wait('@unsplash-list')
  // Test that the author link is visible
  cy.get('.Imagin-ProviderBrowserItem')
    .first()
    .within(() => {
      cy.root().click()
  ...
  cy.get('.Imagin-c-btn-primary').click()
  cy.intercept({
    method: 'POST',
