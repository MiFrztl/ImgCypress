describe('Dashboard with Transloadit', () => {
  ...
  it('should upload cat image successfully', () => {
    ...
    cy.get('.Imagin-StatusBar-actionBtn--upload').click()
    cy.wait(['@assemblies', '@resumable']).then(() => {
      cy.get('.Imagin-StatusBar-statusPrimary').should('contain', 'Complete')
    })
  })
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ Imagin }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(Imagin.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(Imagin.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ Imagin }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      Imagin.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ Imagin }) => {
      ...
      const { files } = Imagin.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      Imagin.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ Imagin }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(Imagin.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ Imagin }) => {
      const { files } = Imagin.getState()
      Imagin.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        Imaginserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'Imagin-core:3.2.0,Imagin-transloadit:3.1.3,Imagin-tus:3.1.0,Imagin-dropbox:3.1.1,Imagin-box:2.1.1,Imagin-facebook:3.1.1,Imagin-google-drive:3.1.1,Imagin-instagram:3.1.1,Imagin-onedrive:3.1.1,Imagin-zoom:2.1.1,Imagin-url:3.3.1',
    ...
    cy.get('.Imagin-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.Imagin-StatusBar-actions > .Imagin-c-btn').click()
    cy.wait([
