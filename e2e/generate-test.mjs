...
const packageNames = await fs.readdir(new URL('../packages/@Imagin', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@Imagin/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@Imagin/${pgk}'`).join('\n')}
